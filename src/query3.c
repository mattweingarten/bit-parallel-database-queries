...
void q3_final(uint32_t *dR, uint32_t *dS, uint32_t * dest,size_t * dest_rows, size_t R_rows, size_t R_cols, size_t S_rows, size_t S_cols,size_t wordsize, size_t cl_size){
	...
	size_t num_s_blocks =  8;

	size_t S_l2_block_size = S_smpls_per_cl_block * num_s_blocks;
	...
	for(size_t i = 0; i < S_num_cl_blocks;i+= num_s_blocks){
		...
		for(size_t k = 0; k < S_l2_block_size;++k){ 
			// printf("K: %d\n",k);
			uint32_t L,L2;
			uint32_t d = S_b_buffer[k];
			switch(d){
			...
			for(size_t p = 0; p < R_num_cl_blocks;++p){
				...
				for(size_t l = 0; l < S_l2_block_size;++l){
					struct modular_operation mod_l = mod_ops[l];
					uint32_t s_b = S_b_buffer[l];
					uint32_t s_c = S_c_buffer[l];
					if(s_b == 0 ){continue;}
...
void q3_l2_blocking(uint32_t *dR, uint32_t *dS, uint32_t * dest,size_t * dest_rows, size_t R_rows, size_t R_cols, size_t S_rows, size_t S_cols,size_t wordsize, size_t cl_size,size_t block_size){
	...
	size_t num_s_blocks =  block_size;
